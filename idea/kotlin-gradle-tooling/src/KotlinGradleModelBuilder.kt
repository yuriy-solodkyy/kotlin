...
class KotlinWithAndroidPluginProjectImportModelProvider : ProjectImportModelProvider {
    ...
    override fun populateProjectModels(
        ...
        println("Requesting Kotlin model in Java mode for: ${(gradleModel as? ProjectModel)?.projectIdentifier?.projectPath}")
        buildController.findModel(gradleModel, KotlinGradleModel::class.java, ModelBuilderService.Parameter::class.java) {
            println("Configuring the parameter")
            it.value =
                NON_ANDROID_GRADLE_PROJECT_ONOLY_MARKER // Empty string tells the builder to build the model only if requested for non-Android module.
        }?.let { model ->
